<template>
  <div class="container mx-auto p-6 pb-10 max-w-4xl">
    <h2 class="text-2xl font-semibold mb-6 text-center">Student Testimonials</h2>

    <div class="overflow-hidden relative">
      <div class="flex transition-transform duration-500" :style="`transform: translateX(-${currentIndex * 100}%);`">
        <div v-for="(testimonial, index) in testimonials" :key="index" class="min-w-full p-8 bg-gray-100 rounded shadow-lg">
          <p class="italic text-gray-800 mb-2">"{{ testimonial.quote }}"</p>
          <p class="text-right text-sm font-medium text-gray-700">– {{ testimonial.author }}</p>
        </div>
      </div>
